...
There is information about people and places. Only anonymised data stored in th
e sample public dataset. Nothing to uniquely identify the demographics or 
...
success is 'Hires'. We will visualise hires against time, against demographics 
and show a map where the largest number of hires is displayed.
...
* First time user creates account using website or app. They add their payment details and buy a short pass or a long term membership. Customer charged.
* User undocks bike by scanning QR code
* User goes on bike trip
* User docks bike
* Citi Bike checks the duration of the hire and charges any payment due in excess of user type allowances
...
missing records were detected. Some parts directly are supplied by the user such
as Gender/Year of birth. The preliminary analysis revealed over that for over 6%
of trips the gender of the user was unknown. A couple of entries gave year of 
birth of the person hiring the bike as being in the 1880s.
...
Citi Bike bikes hire process involves users using their mobile phones to 'unlock'
bikes. Citi Bike will be unable to determine from this dataset whether there are 
people who would like to use the service but do not have a mobile phone or can't
use the apps involved. There is no information on failed attempts e.g. technical faults or not enough bikes available.
...
sample is from a specific subcategory of e.g. stations/bikes/users or other data 
Citi Bike holds but not made public. We are unable to claim how well this sample 
represents the full dataset to which we do not have access.

Possible undercoverage bias? The sample is only for over 4,000 trips over the 
course of 2018 for just 10 different bikes and only around 50 stations. We can
only examine that particular year without examining whether there are 
year-to-year trends such a seasonal variations.

Response bias: There are some questionable entries in birth year information 
comes from the user so might have been an error on their part mistyping, bug in 
app or deliberately choosing the wrong year.
...
A preliminary analysis was used for examining the initial quality of the 
data. As a result a separate cleaning script was created to small amount of cleaning/wrangling was done to add extra time-related columns to the data for 
convenience when visualising it as plots.
...
```
This line plot shows the pattern of hires with one value for every day of 2018.
In general terms we see hires at their lowest in winter months, higher in summer 
with a peak in August. Citi Bike could do promotions at quieter times of year to 
try and even out usage. Or look at the top days and was there anything Citi Bike 
did to boost hires and to repeat anything similar. We will pull out tables of 
high and low hires.

##### Top 5 hires dates
```{r}
top_5_hire_dates <- citibike_hires_by_date %>%
  arrange(desc(hires)) %>%
  head(5)
...
```
##### Lowest hires dates
There are some days where hires were barely measurable with only one hire.
Citi Bike should check their records for reasons why this may be. Was the 
weather particularly bad, were there road problems, were Citi Bike having 
...
August. Usage then drops very sharply in September and carries on dropping 
til the end of the year.

The drop is interestingly not as steep from September to October. Citi Bike 
...
activity or an influx of e.g. shortly after start of academic term for students 
in New York?
...
This line bar plot shows the pattern of hires for each day of the week and also 
highlights which days are weekend usage.

We see that hires are lower at weekends than weekdays. Resources are less 
heavily used on weekends so there should be scope for promoting more leisure 
trips within the current service capacity. Could leisure promotions at weekends 
help compensate for loss of local weekday commuters. There is no immediately 
obvious reason why Tuesday should be higher than the others.
...
citibike_hires_by_hour_of_day %>%
  ...
  labs(
    title = "Hires by Hour of the Day",
    subtitle = "Sample data from 2018",
    x = "\nHour of Day",
    y = "Number of Hires\n",
    fill = "Office Hours"
...
```
This bar plot shows hires for all 24 hours of the day.

As migh be expected this shows peak movement before and after traditional 
"office hours" 9-5 with an hour or two either side. We must remember this was 
before COVID-19 and many more people commuted into city centre offices for work. Hires are remarkably even from 10am to 3pm. Thse are the dip in the middle. CityBike might like to experiment with promotions to encourage more 
use if bike trip started during these hours. Not really any sign of workers going on bike trips during lunch breaks. Perhaps incentivise tourists able to move at these times. Very little usage from 1-4am, could Citi Bike target shift workers e.g. health/emergency workers or the entertainment sectors.

### Demographics: Hires by User Type 
...
Subscribers are those who paid for a full year of use. It suggests that a 
significant group of those who took out subscriptions in August or October the 
previous year 2017 did not choose to renew at this time. Were there any 
special promotions those months in the previous year? Is there a cyclical drop 
showing the same pattern in later years?

The proportion of customers seems highest in the summer months. Are people more
likely to experiment with short passes for the Citi Bike service in good weather?
Are tourists accounting for these increases? Maybe on signing up for the 
service Citi Bike could ask for optional data around what use the user had 
planned.
...
ggsave("analysis_and_documentation/images/plot_06_citibike_hires_user_age.png")
```
...
```{r}
citibike_hires_station_locations <- tibble(citibike_hires) %>%
  select(start_station, start_lat, start_long) %>%
  group_by(start_station, start_lat, start_long) %>%
  summarise(number_of_hires = n()) %>%
  ungroup()

citibike_hires_station_start_map <- leaflet(citibike_hires_station_locations) %>%
...
#ggsave("analysis_and_documentation/images/plot_07_citibike_hires_station_start_map.png")
```
...
ggsave("analysis_and_documentation/images/plot_08_citibike_hires_duration.png")
```
...
trips lasting less than 10 minutes. Suggests users find Citi Bike worthwhile even 
...
cheaper and easier options for impromptu "day trips".
* Overwhelming majority of users are subscribers with yearly membership 
suggesting satisfaction with the service Citi Bike offers. Use this vote of 
confidence to tempt potential new users using incentives to try 
a short pass first - discounts on full membership for new customers.
* Males use Citi Bike far more than females. Quick win could be to attempt to 
grow female usage through trying targeted marketing campaigns and working with 
female groups in New York City.
* Developers/designers should check whether the user interface provided could be 
adjusted to make birth_year optional to see if it improves quality of age data 
reducing those deliberately choosing to give a false year.

## Technical Appendix: Data types 
<!-- (1.5) 
