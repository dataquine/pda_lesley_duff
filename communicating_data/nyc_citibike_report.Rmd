...
accordingly. Date and time-related data could be incorporated into the scheduling of any promotional campaigns. 
...
use the apps involved. Similarly the information we have is for only where bikes 
have actually been hired successfully, there is no information on failed 
attempts e.g. technical faults or not enough bikes available etc. these would 
require futher separate research.
...
represents the full dataset to which we don not have access.

Possible undercoverage bias? The sample is only for 4268 trips over the course 
of 2018 for just 10 different bikes and only around 50 stations. We can only 
make observations for that particular year without examining whether there are 
year-to-year trends.

Response bias: There are some questionable entries in birth year information 
comes from the user so might have been an error on their part mistyping or 
