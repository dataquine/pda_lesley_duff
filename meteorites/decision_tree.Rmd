...
feature (predictor variable) and value used for the split e.g. for the root node
(Found, 0.85, 100%), The second line contains probability of a Found result expressed as a decimal i.e. 0.85, the third line is the percentage of data 
points which pass through this node.
From root node where data meets the split `year < 1936` for 
years older than 1936, it follows the left branch, years from 1936 onwards it follows the right branch. Once you reach a leaf node,  the node tells you the predicted outcome, in this case 'Fell' or 'Found'.

So from root if year was >= 1936 we'd take the right branch to
Found, 0.93, 79%
This tells us a very high percentage of meteorites discovered 1936 or later are 
predicted to have been discovered **after** its impact than seen falling.

For the left branch from root

1. Root Node take 'year < 1936'(dark green) - take left branch
2. Node: Found, 0.56, 21% (light green)
The split is now on mass rather than year.
For mass in grammes < 11000 (11e+3) take left branch
3. Node: Fell, 0.42, 10% (light blue)
For the first time we see 'Fell' as predicted more common. 
4. Leaf Node: Fell, 0.21, 4%. (dark blue)
So Fell is predicted to more likely for lighter meteorites < 11000g, where the 
year is older than 1880 
...
So where 'Fell' was predicted this was true 32 times (True Positive), false 16 (False Positive)

Prediction of Found and was actually Fell 119 is a False Negative

Our correctly predicted values are higher than the incorrect ones. This is
reflected in the probability of our prediction being correct as over 85%.

Our sensitivity (AKA true positive rate) of 21% is the proportion of 
actual positive cases that are correctly identified as positive.

Prediction of Found and was actually Found 795 is a True Negative, this value 
is very high.
Our specificity (AKA true negative rate) of over 98%  proportion of actual 
