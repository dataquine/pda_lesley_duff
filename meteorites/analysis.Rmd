...
2. If year given is newer than dataset (newer than 2013) It would be maybe 
better to indicate that no data is available rather than give the impression 
there might be no meteors that year. Test whether year supplied is more recent 
than 2013 create new output to indicate no data available and that year should 
be <= 2013.

```{r, error=TRUE}
test_that("Years must be <= 2013", {
  expect_equal(get_meteorite_count(meteorite_data, 2020), 
               "We have no data about meteorites for years later than 2013")
})
```
